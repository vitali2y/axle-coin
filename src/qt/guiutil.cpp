...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AXLE address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no AXLE: URI
...
{
    // Convert axle:// to axle:
    //
    //    Cannot handle this later, because axle:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open axle.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AXLE.lnk";
}
...
{
    return GetAutostartDir() / "axle.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a axle.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AXLE\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the axle app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add axle app to startup item list
