...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AXLE.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal axle:URI (axle:XyZ...?message=example)
            {
...
static void NotifyzAXLEReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAXLEReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAXLEReset.connect(boost::bind(NotifyzAXLEReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAXLEReset.disconnect(boost::bind(NotifyzAXLEReset, this));
}
