...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zAXLE spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zAXLE spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zAXLE serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AXLEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AXLEMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AXLEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("axle-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        CBlockIndex* pindexPrev = chainActive.Tip();
        if (!pindexPrev) {
            LogPrintf("AXLEMiner bailing, no pindexPrev\n");
            continue;
        }
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
        if (!pblocktemplate.get()) {
            LogPrintf("AXLEMiner bailing, no pblocktemplate got\n");
            continue;
        } else {
            LogPrintf("AXLEMiner proceeding with pblocktemplate.\n");
        }
        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrintf("AXLEMiner skipping PoS section 4\n");
        //Stake miner main
        if (fProofOfStake) {
        ...
        LogPrintf("Running AXLEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("Running AXLEMiner with hashTarget %0x\n", hashTarget.GetCompact());
        while (true) {
