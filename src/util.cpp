...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "axle" is a composite category enabling all AXLE-related debug output
            if (ptrCategory->count(string("axle"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AXLE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AXLE
// Mac: ~/Library/Application Support/AXLE
// Unix: ~/.axle
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AXLE";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "axle.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "axled.pid"));
